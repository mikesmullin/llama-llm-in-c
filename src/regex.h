...
typedef struct regex_t {
  unsigned char type; /* _CHAR, STAR, etc.                      */
  union {
...
int re_match_group(const char* start, const char* end, const char* haystack, char* dst) {
  int oa, la, ob, lb;
  if (-1 != (oa = re_match(start, haystack, &la))) {
    if (-1 != (ob = re_match(end, haystack + oa + la, &lb))) {
      memcpy(dst, haystack + oa + la, ob);
      dst[ob] = 0;
      return oa + la + ob + lb;
    }
  }
